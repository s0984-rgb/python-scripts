...
parser.add_argument("-a", "--action", help="Action to commit on tar", choices=['archive', 'extract'], required=True, type=str)
parser.add_argument("-c", "--certificate_path", help="Path to the certificate to use for S3 endpoint", default="/tmp/ca.crt", required=False, type=str)
parser.add_argument("-b", "--bucket", help="Name of the S3 bucket to upload to", required=True, type=str)
parser.add_argument("-d", "--directory", help="Directory to run archive/extraction", required=True, type=str)
parser.add_argument("-i", "--key_id", help="S3 Key ID to use", required=True, type=str)
parser.add_argument("-k", "--key_secret", help="S3 Key Secret to use", required=True, type=str)
parser.add_argument("-n", "--name", help="Name of the archive *Appended with timestamp + '.tgz'*", default="archive.tar.gz", required=False, type=str)
parser.add_argument("-s", "--state_file", help="State file to use", default="archived_files.state", required=False, type=str)
parser.add_argument("-u", "--endpoint_url", help="endpoint url for s3 upload", required=True, type=str)
